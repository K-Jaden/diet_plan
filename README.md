...
### 3. 🌐 공공데이터 API 연동 및 지연 로딩 (Lazy Loading)
- **식품안전나라 API**: 로컬 DB에 일치하는 레시피가 부족할 경우, 실시간으로 식품안전나라 API를 호출하여 최신 레시피를 가져옵니다.
- **Lazy Loading 아키텍처**: 최초 실행 시 모든 데이터를 받아오지 않고, 유저가 특정 메뉴를 검색하거나 AI가 메뉴를 추천했을 때 DB에 없으면 그때그때 네트워크를 통해 가져와 영구 저장(Room DB)합니다.

### 4. 🔒 사용자 계정 관리 및 개인화 기능 (New!)
- **회원가입 및 로그인**: Room DB를 이용한 로컬 계정 관리 시스템이 내장되어 있습니다. 비밀번호는 SHA-256 해시 함수로 암호화되어 안전하게 보관됩니다.
- **맞춤형 신체 정보 및 칼로리 계산**: 사용자의 성별, 나이, 키, 몸무게, 활동량 및 식단 목표(다이어트, 벌크업, 유지)를 입력받아 Mifflin-St Jeor 공식 기반으로 일일 권장 칼로리(TDEE)를 자동 계산하여 반영합니다.
...
- **Security**: SHA-256 Hashing for Password
...
- `db/`: Room Entity, DAO, Repository (레시피 데이터, 식단, 회원가입 유저 DB)
- `MainActivity.kt`: Compose NavHost 기반의 전반적인 UI 라우팅 및 뷰 구성 
- `AuthScreens.kt`: 로그인, 회원가입 등 인증 처리 화면 구성

---

## 📝 향후 개선 계획 (TODO)
- 사용자의 일일 실제 섭취 기록(Record) 기능 및 그래프 시각화 추가
- 서버(Firebase 또는 자체 백엔드) 연동을 통한 사용자 데이터 클라우드 동기화 